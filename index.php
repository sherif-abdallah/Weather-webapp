...
$location_name = explode('","cod":' , explode('"name":"', file_get_contents($my_city_api_url))[1])[0];
$description = explode('","icon' , explode(',"description":"', file_get_contents($my_city_api_url))[1])[0];
$temp = explode(',"' , explode('"temp":', file_get_contents($my_city_api_url))[1])[0] - 273.15;
$temp_min = explode(',"' , explode('"temp_min":', file_get_contents($my_city_api_url))[1])[0] - 273.15;
$temp_max = explode(',"' , explode('"temp_max":', file_get_contents($my_city_api_url))[1])[0] - 273.15;
$feels_like =  explode(',"' , explode('feels_like":', file_get_contents($my_city_api_url))[1])[0] - 273.15;
$pressure = explode(',"' , explode('pressure":', file_get_contents($my_city_api_url))[1])[0];
$humidity = explode('},' , explode('humidity":', file_get_contents($my_city_api_url))[1])[0];
$wind = explode(',"' , explode('"wind":{"speed":', file_get_contents($my_city_api_url))[1])[0];
?>
<!DOCTYPE html>
<html lang="en">
<head>
      ...
      <div class="modal-body">
        <form method="POST" action="change_location.php">
          <div class="form-group">
            <input autocomplete="off" spellcheck="false" type="text" name="city" class="form-control" id="recipient-name" placeholder="Enter City Name">
          </div>
      </div>
